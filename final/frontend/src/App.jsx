...
const globalStyles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  :root {
    ...
    --sky: #6b8fa3; --gold: #c8a96e; --fog: #e8dfd0; --shadow: rgba(61,43,31,0.12);
  }
  html, body { height:100%; font-family:'DM Sans',sans-serif; background:var(--cream); color:var(--bark); }
  @keyframes fadeUp { from{opacity:0;transform:translateY(24px)} to{opacity:1;transform:none} }
  @keyframes drift  { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-8px)} }
  @keyframes fadeIn { from{opacity:0} to{opacity:1} }
  @keyframes spin   { to{transform:rotate(360deg)} }
  .modal-overlay {
    position:fixed;inset:0;background:rgba(61,43,31,0.5);z-index:999;
    display:flex;align-items:center;justify-content:center;animation:fadeIn .2s ease;padding:20px;
  }
  .modal-box {
    background:white;border-radius:16px;padding:36px;width:100%;max-width:520px;
    box-shadow:0 20px 60px rgba(0,0,0,0.2);animation:fadeUp .3s ease;max-height:90vh;overflow-y:auto;
  }
  .tab-btn { padding:8px 20px;border:none;background:transparent;cursor:pointer;font-family:'DM Sans',sans-serif;font-size:14px;color:#8a7060;border-bottom:2px solid transparent;transition:all .2s; }
  .tab-btn.active { color:var(--bark);border-bottom-color:var(--bark);font-weight:500; }
  .cabana-img { width:100%;height:180px;object-fit:cover;background:#e8dfd0; }
  .cabana-img-placeholder { width:100%;height:180px;display:flex;flex-direction:column;align-items:center;justify-content:center;background:linear-gradient(135deg,var(--fog),#d5c9b8);color:#8a7060;gap:8px; }
...
const Toast = ({ msg, type, onClose }) => {
  ...
  return (
    <div style={{ position:"fixed",top:"24px",right:"24px",zIndex:2000,background:c.bg,
            ...
            {entretenimientos.map(e => {
              ...
              return (
                <button key={e.id} onClick={()=>toggleEnt(e.id)}
                  style={{ padding:"6px 14px",borderRadius:"20px",fontSize:"13px",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",border:"1.5px solid",transition:"all .2s",
          ...
          <nav style={{ display:"flex",gap:"4px" }}>
            {[{ id:"dashboard", label:"Dashboard" }, { id:"catalogo", label:"Cabañas" }].map(t => (
              <button key={t.id} onClick={()=>setPaginaActual(t.id)}
                style={{ padding:"6px 16px",borderRadius:"8px",border:"none",fontSize:"14px",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",transition:"all .2s",
...
const LoginPage = ({ onLogin, onGoRegister }) => {
  ...
  return (
    <div style={{ minHeight:"100vh",display:"flex",background:"var(--cream)",fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ flex:1,display:"flex",flexDirection:"column",justifyContent:"center",padding:"64px 72px",animation:"fadeUp .7s ease both" }}>
        ...
        <div style={{ marginBottom:"20px" }}>
          ...
          <div style={{ position:"relative" }}>
            <div style={{ position:"absolute",left:"16px",top:"50%",transform:"translateY(-50%)",color:focused==="username"?"var(--moss)":"#b0a090" }}><UserIcon /></div>
            <input name="username" value={form.username} onChange={e=>setForm(f=>({...f,username:e.target.value}))}
              ...
              style={{ width:"100%",padding:"14px 16px 14px 44px",border:`1.5px solid ${focused==="username"?"var(--moss)":"var(--fog)"}`,borderRadius:"10px",fontSize:"15px",outline:"none",background:"white",color:"var(--bark)",fontFamily:"'DM Sans',sans-serif" }} />
          </div>
        </div>
        <div style={{ marginBottom:"28px" }}>
          ...
          <div style={{ position:"relative" }}>
            <div style={{ position:"absolute",left:"16px",top:"50%",transform:"translateY(-50%)",color:focused==="password"?"var(--moss)":"#b0a090" }}><LockIcon /></div>
            <input type="password" name="password" value={form.password} onChange={e=>setForm(f=>({...f,password:e.target.value}))}
              onFocus={()=>setFocused("password")} onBlur={()=>setFocused(null)} onKeyDown={e=>e.key==="Enter"&&handleSubmit()} placeholder="••••••••"
              style={{ width:"100%",padding:"14px 16px 14px 44px",border:`1.5px solid ${focused==="password"?"var(--moss)":"var(--fog)"}`,borderRadius:"10px",fontSize:"15px",outline:"none",background:"white",color:"var(--bark)",fontFamily:"'DM Sans',sans-serif" }} />
          </div>
        </div>
        ...
        <button onClick={handleSubmit} disabled={loading}
          style={{ width:"100%",padding:"15px",background:loading?"var(--fog)":"var(--bark)",color:loading?"#8a7060":"var(--cream)",border:"none",borderRadius:"10px",fontSize:"15px",fontWeight:500,cursor:loading?"not-allowed":"pointer",fontFamily:"'DM Sans',sans-serif" }}>
        ...
        <p style={{ textAlign:"center",marginTop:"32px",color:"#c0b0a0",fontSize:"12px" }}>Sistema de gestión · Mar Azul © 2025</p>
      </div>
    </div>
  );
};
...
const RegistroPage = ({ onBack }) => {
  ...
  const handleSubmit = async () => {
    if (!form.nombre||!form.apellido||!form.email||!form.username||!form.password||!form.fechaNacimiento) { setError("Completa todos los campos obligatorios."); return; }
    if (form.password.length < 8) { setError("La contraseña debe tener al menos 8 caracteres."); return; }
    setLoading(true); setError("");
    try {
      await api("/api/auth/registro",{method:"POST",body:JSON.stringify({...form,telefono:form.telefono||"000",pais:form.pais||"Colombia"})});
      setOk(true);
  ...
  return (
    <div style={{ minHeight:"100vh",background:"var(--cream)",display:"flex",alignItems:"center",justifyContent:"center",padding:"40px",fontFamily:"'DM Sans',sans-serif" }}>
      <div style={{ width:"100%",maxWidth:"480px",animation:"fadeUp .6s ease both" }}>
        <button onClick={onBack} style={{ background:"none",border:"none",cursor:"pointer",color:"#8a7060",fontSize:"14px",marginBottom:"24px",display:"flex",alignItems:"center",gap:"6px",padding:0 }}>← Volver al inicio</button>
